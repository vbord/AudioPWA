/* ================================================
   Custom Audio Player (original, speed-fixed only)
   ================================================ */
...
/* ================================================
   SPEED FIX: SAFE SEEK HELPER
   (prevents readyState dropping to 1)
   ================================================ */
...
---------------------------------- */
btnBack.onclick = () => {
    safeSeek(Math.max(0, audio.currentTime - 15));
};

/* --------------------------------
   SEEK FORWARD 30s (FIXED)
---------------------------------- */
btnForward.onclick = () => {
    if (!audio.duration) return;
    safeSeek(Math.min(audio.duration, audio.currentTime + 30));
};
...
function moveDrag(e) {
    ...
    timeDisplay.textContent =
        `${formatTime(previewTime)} / ${formatTime(audio.duration)}`;
}
...
function endDrag(e) {
    ...
    const finalTime = pct * audio.duration;
    safeSeek(finalTime);
